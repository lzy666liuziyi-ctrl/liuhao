import { findAudioByInstrumentKeyword } from '../data/audioSources';
...
export function generateMusic(options: GenerateOptions): GeneratedMusic {
  ...
  const matched = options.instruments.map(findAudioByInstrumentKeyword).find(Boolean)
    ?? { id: 'guqin-demo', displayName: '古琴演示音源', audioUrl: '/samples/guqin/guqin-demo.mp3', personalUseOnly: true };
  ...
  return {
    ...
    audioSourceId: matched.id,
    audioSourceName: matched.displayName,
    personalUseOnly: matched.personalUseOnly,
    sampleMode: 'single-file'
  };
}
